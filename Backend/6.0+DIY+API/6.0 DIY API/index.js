...
{
 var jlen = jokes.length;
 var ri = Math.floor(Math.random(0,1)*jlen);
 var joke = jokes[ri];
 console.log(joke);
 res.json(joke);
});
...
app.get("/joke/filter", (req,res) =>
{
  ...
  console.log(filters); 
  var foundJokes = jokes.filter((joke) => joke.jokeType === filters.type);
  res.json(foundJokes);
});
...
app.put("/jokes/:id", (req, res) =>
{
  ...
  var searchIndex = jokes.findIndex((joke) => joke.id == jokeId);
  const updatedJoke = {
    id: jokeId,
    jokeText: req.body.text,
    jokeType: req.body.type
...
app.delete("/jokes/:id", (req, res)=>
{
  ...
  if(searchIndex > -1)
  {
    ...
    console.log("Could not find joke with that id!");
    res.status(404)
    .json({error: `Joke with id : ${jokeId} not found.`});
  }
});
